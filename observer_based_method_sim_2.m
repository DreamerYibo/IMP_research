...
GAMMA = [];
for (i=1:N)
    [PI_temp, GAMMA_temp] = IMP_full_solver(A,B,C,zeros(n,2),Q_c_bar((2*i-1):(2*i),:), S_bar);
    PI = [PI; PI_temp]; GAMMA = [GAMMA; GAMMA_temp];
end
L_c_bar = GAMMA-K_c*PI;
...
for k= 2:(t_max/dt)
    u = K_c*Xi_c(:,k-1)+L_c*Omega_(:,k-1)+L_c_bar*Omega_bar;
    % u = B_c*K_c*Xi_c(:,k-1);
...
for k = 1:t_max/dt
    u = K_c*Xi_c(:,k)+L_c*Omega_(:,k)+L_c_bar*Omega_bar;
    for i = 1:N 
        E_norm(i,k) = norm(C*X_c((i-1)*n + 1:  i*n, k) + Q_c((i-1)*2+1:i*2, :)*Omega_(:,k)+Q_c_bar((i-1)*2+1:i*2, :)*Omega_bar);
...
ylabel("$\parallel Cx_i-Q_i v-\bar{v} \,\,\parallel$", 'interpreter', 'latex')
