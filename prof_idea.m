%% Try variable radius and see if the magnitute of the input become lower
...
F = A+B*K-G*C; %observer design
F_c = kron(eye(N), F);


phi_list = zeros(1,N); %%initial phases
...
L_2_bar= Sigma*S_bar-G*C*Pi-F*Sigma;
L_1c_bar = kron(eye(N), L_1_bar);
L_2c_bar = kron(eye(N), L_2_bar);
...
A_closed = [A_c, B_c*K_c, B_c*L_1c, B_c*L_1c_bar;
            G_c*C_c, F_c, L_2c, L_2c_bar;
            zeros(2*N, 2*n*N), kron(eye(N), S), zeros(2*N, 2*N);
            zeros(2*N, 2*n*N), zeros(2*N, 2*N), kron(eye(N), S_bar)];


f1 = @(t) ((2>t) && (t>=0))*2+((10>t) && (t>=2))*4+((t>=10))*7;
f2 = @(t) (t<= 0.5*t_max)* (t/(0.5*t_max))* l + (t> 0.5*t_max)*l;

Omega_c(:,1) = reshape([l*cos(phi_list); l*sin(phi_list)], [2*N,1]);
Omega_c(3:4,1) = f1(0)* [cos(phi_list(2)); sin(phi_list(2))];
Omega_c(5:6,1) = f2(0)* [cos(phi_list(3)); sin(phi_list(3))];
...
for k= 2:(t_max/dt)
    ...
    % Force omega to be user specified
    Omega_c(3:4,k) = f1((k-1)*dt)* [cos(phi_list(2)+w*(k-1)*dt); sin(phi_list(2)+w*(k-1)*dt)];
    Omega_c(5:6,k) = f2((k-1)*dt)* [cos(phi_list(3)+w*(k-1)*dt); sin(phi_list(3)+w*(k-1)*dt)];
    state_cl(2*N*n+1 : 2*n*N+2*N) = Omega_c(:,k);
...
for k = 1:t_max/dt
    ...
    for i = 1:N 
        ...
        E_norm(i,k) = norm(C*X_c((i-1)*n + 1:  i*n, k) - Omega_c(1:2 ,k)-Omega_bar);
