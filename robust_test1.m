% Unknown A dynamics. State feedback case
...
map_size = [100,100]; % cm
N=6;n=4; %agent 1 original L_bar, agent i L_bar + delta_L_i
w=1;
l = 7;
...
d_L = zeros(2,2,2*2); %all possible delta L
d_L(:,:,1) = reshape([1 0 0 0], [2,2]); 
d_L(:,:,2) = reshape([0 1 0 0], [2,2]); 
d_L(:,:,3) = reshape([0 0 1 0], [2,2]); 
d_L(:,:,4) = reshape([0 0 0 1], [2,2]); 

%L is calculated by the real model A. It is only available for agent 6
Pi = zeros(n,2);
Gamma = zeros(2,2);
...
L_c = blkdiag(L_bar, L_bar+d_L(:,:,1), L_bar+d_L(:,:,2), L_bar+d_L(:,:,3), L_bar+d_L(:,:,4), L);
...
for k= 2:(t_max/dt)
    ...
    if (mod(t,1) < dt) %calculate grammian if (mod(t,1) == 0) does not work, very strange.
        k2 = k;
        k1 = k-1/dt;
...
for j=(1:N)
    disp(num2str(j)+":  Pi_tilde obtained by sylvester equation" )
    temp_sol = linsolve(kron(S', eye(n,n))-kron(eye(2,2)', A+B*K), reshape(B*L_c(2*(j-1)+1:2*j, 2*(j-1)+1:2*j), [n*2,1]));
    disp(reshape(temp_sol, [n,2]))

    disp(num2str(j)+": Pi_tilde obtained by esitimation" )
    disp(Gramm_c(n*(j-1)+1: n*j, end-1:end))
end

d_CPi_tilde = zeros(2,2,4);

for j=(2:5)
    d_CPi_tilde(:,:,j-1) = C*(Gramm_c(n*(j-1)+1: n*j, end-1:end)) -C*(Gramm_c(1: n, end-1:end));
end

alp = zeros(4,1); 
alp = - pinv(reshape(d_CPi_tilde, [2*2,4]))* reshape((C*Gramm_c(1: n, end-1:end)-eye(2,2)), [4,1]);
L_mod = L_bar + reshape(reshape(d_L, [2*2,4])*alp, [2,2])

disp("L_mod calculated by IMP equation")
disp(L)


%Prof's idea: test if we can get delta_A
Pi_tilde_test_1 = Gramm_c(1:4,end-1:end);
Pi_tilde_test_2 = Gramm_c(9:12,end-1:end); %third agent

Pi_merge=[Pi_tilde_test_1, Pi_tilde_test_2];
A_estimate = (-B*[L_bar, L_bar+d_L(:,:,2)]+Pi_merge*kron(eye(2,2),S)-B*K*Pi_merge)*inv(Pi_merge);
A_estimate
