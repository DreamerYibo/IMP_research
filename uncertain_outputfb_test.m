% Unknown A B C dynamics. Output feedback feedback case. 

% Change A such that the center can be shifted
clc;clear all;
% load('outputfb_bad model', 'A', 'B', 'C')
...
L_1_bar=Gamma_bar - K*Sigma_bar;
L_2_bar= Sigma_bar*S-F*Sigma_bar-G*C_bar*Pi_bar;
...
pz_pl = - kron(eye(2), [C_bar, zeros(2,n)])*inv(sylv_sq_mat)*kron(eye(2,2), blkdiag(B_bar, eye(n,n))); %pd z/pd l based on the nominal model
nominal_pz_pl = pz_pl;

A_f_actual = [A, B*K;
            G*C, kron(eye(N), F)];
actual_sylv_sq_mat = -kron(S', eye(2*n,2*n))+kron(eye(2,2)', A_f_actual);
actual_pz_pl = - kron(eye(2), [C, zeros(2,n)])*inv(sylv_sq_mat)*kron(eye(2,2), blkdiag(B, eye(n,n))); 

if (any(real(eig(A_f_actual)) > 0))
...
for k= 2:(t_max/dt)
    ...
    if (mod(t,h_gram) < dt) %calculate grammian if (mod(t,1) == 0) does not work, very strange.
        ...
        if (counter > 1 && counter - update_L_index > 1)
            ...
            if (all( abs(Gramm_c(:,end-1:end) - Gramm_c(:,end-3:end-2))./norm(Gramm_c(:,end-1:end))< 0.01) )
                ...
                l_temp = reshape([L_1_bar;L_2_bar], [2*(n+2),1]);
                if (update_L_index == 0 || norm(l_temp - l_list(:,end)) > 0.01)
                    ...
                    pz_pl = reshape(nominal_pz_pl(:)+delta_pz_pl_update(:), [4,2*(n+2)]);
                    %debug
                ...
                l_1 = reshape([L_1_bar;L_2_bar], [2*(n+2),1]);
                pe_pz = 2*z_1' + rt_op;
                pe_pl = pe_pz*pz_pl;
                delta_l = -pe_pl';
                ...
                e_1 = norm(z_1-[[1 0], [0 1]]')^2;
                e_2 = norm(z_2-[[1 0], [0 1]]')^2;
                ...
                d_L = reshape(l_2-l_1, [2+n,2])  
                L_1_bar = L_1_bar + d_L(1:2,:);
